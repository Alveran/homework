 ...
 * bmp_width - Bildbreite (in Pixel)
 * bmp_height - Bildhoehe (in Pixel)
 * x - X-Koordinate im BMP.
 * y - Y-Koordinate im BMP.
 * color - Pixelfarbe.
 */
void bmp_set_pixel(uint32_t * data, int bmp_width, int bmp_height, int x, int y,
                   uint32_t color) {
  // TODO: implementieren!
}
 ...
 * bmp_height - Bildhoehe (in Pixel)
 */
void bmp_create(char *bmp_name, uint32_t * data, int bmp_width, int bmp_height) {
  FILE *bmp = fopen(bmp_name, "w");
  if (bmp == NULL) {
    fprintf(stderr, "\nbmp Bilddatei konnte nicht geoeffnet werden!\n");
    exit(EXIT_FAILURE);
  }
  bitmap_file_header(bmp, bmp_width, bmp_height);
  bitmap_info_header(bmp, bmp_width, bmp_height);
  for (int y = 0; y < bmp_height; y++) {
    for (int x = 0; x < bmp_width; x++) {
      bmp_write_N_byte(bmp, data[y * bmp_width + x], 4);
    }
  }
  fclose(bmp);
}
